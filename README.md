<div align="center">

# [WAV0 AI - The Studio for Everything Audio](https://wav0.app)

[![License: AGPL-3.0](https://img.shields.io/badge/License-AGPL--3.0-blue.svg)](https://www.gnu.org/licenses/agpl-3.0)
[![Website](https://img.shields.io/badge/website-wav0.app-blue)](https://wav0.app)
[![Discord](https://img.shields.io/badge/discord-join%20us-5865F2)](https://wav0.app/discord)
[![X Follow](https://img.shields.io/twitter/follow/wav0ai?style=social)](https://x.com/wav0ai)
[![Contributors](https://img.shields.io/github/contributors/fluid-tools/wav0)](https://github.com/fluid-tools/wav0/graphs/contributors)
[![Stars](https://img.shields.io/github/stars/fluid-tools/wav0)](https://github.com/fluid-tools/wav0/stargazers)
![WAV0](https://bfq19ar33r7ynbpv.public.blob.vercel-storage.com/og.png)

</div>

## 🎯 What is WAV0?

> **WAV0 is an AI-Native DAW (Digital Audio Workstation) for Sound Design and Production.** Fast iterations, collaborative workflows, and high-quality production for everyone.

> **Comes with the fastest producer you'll ever meet. Works in your browser, no downloads.**


## 🎯 Who is this for?

> Anyone who wants to create and edit high-quality audio (for songs/podcasts/videos/etc).

### Musicians of All Levels
- **Beatmakers**: Build beats, loops, and samples faster than traditional methods
- **Songwriters**: Prototype ideas and collaborate on tracks in real-time
- **Sound Designers**: Create custom effects and audio assets for any project
- **Indie Artists**: Professional-quality production without expensive software

### Creators of All Kinds
- **YouTube Creators**: Generate custom background music, sound effects, and intros in seconds
- **Twitch Streamers**: Create instant soundpacks for alerts, transitions, and ambient music
- **TikTok Creators**: Produce viral-ready audio that matches your content style
- **Podcasters**: Professional intro/outro music and sound design
- **Game Developers**: Create dynamic soundtracks and audio effects

> **Note**: You don't need to be a producer or have ever opened Logic Pro, FL Studio, or Audacity to create and edit high-quality audio (for songs/podcasts/videos/etc).


## 🚀 Quick Start

Visit [wav0.app](https://wav0.app) to get started.


## 🗺️ Roadmap

- Coming Soon

### Quick Start for Contributors

1. **Read our [Contributing Guide](.github/CONTRIBUTING.md)** - Comprehensive guide to getting started
2. **Join our [Discord](https://wav0.app/discord)** - Connect with other contributors and get help
3. **Check our [Issues](https://github.com/fluid-tools/wav0/issues)** - Find something to work on
4. **Fork and contribute** - Make your first contribution!

### Areas We Need Help With

- 🎵 **Audio Engine**: Improve playback, effects, and processing
- 🤖 **AI Integration**: Enhance music generation capabilities  
- 🎨 **UI/UX**: Make the interface more intuitive and accessible
- 📱 **Mobile**: Optimize for mobile devices and touch interfaces
- 🧪 **Testing**: Add comprehensive test coverage
- 📚 **Documentation**: Improve guides and API documentation
- 🌐 **Internationalization**: Help translate WAV0 to other languages

### Development and Self-Hosting

- **Contributing**: See [CONTRIBUTING.md](.github/CONTRIBUTING.md) for detailed setup instructions and [issues](https://github.com/fluid-tools/wav0/issues) to find something to work on.
- **Self-Hosting**: Guide coming soon.

## Mission

We are redefining how artists and creators generate, shape, and collaborate on sound. 

WAV0 merges systems-level engineering, creative tooling, and human-centered design to unlock faster, more fluid, and more expressive workflows. 

Our goal is to make high-quality audio production accessible to everyone through the power of AI.


## 🤝 Sponsors & Partners

<table>
  <tr>
    <td align="center" style="padding-right: 20px;">
      <a href="https://vercel.com/oss">
        <img alt="Vercel OSS Program" src="https://vercel.com/oss/program-badge.svg" />
      </a>
    </td>
    <td align="center">
      <a href="https://elevenlabs.io/text-to-speech">
        <img src="https://eleven-public-cdn.elevenlabs.io/payloadcms/cy7rxce8uki-IIElevenLabsGrants%201.webp" alt="Text to Speech" style="width:250px">
      </a>
    </td>
  </tr>
</table>
