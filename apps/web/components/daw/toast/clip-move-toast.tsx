							...
							if (t.id === latestMove.toTrackId) {
								return {
									...
									clips: (t.clips ?? []).filter(
										(c) => c.id !== latestMove.clipId,
									),
								};
							}
							if (t.id === latestMove.fromTrackId) {
								const clipOnTarget = prev
									.find((x) => x.id === latestMove.toTrackId)
									?.clips?.find((c) => c.id === latestMove.clipId);
								if (!clipOnTarget) return t;
								const restoredClip = {
									...clipOnTarget,
									startTime: latestMove.fromStartTime,
								};
								...
								if (latestMove.automationData) {
									const env = restored.volumeEnvelope || {
										enabled: true,
										points: [],
										segments: [],
									};
									restored.volumeEnvelope = {
										...
										points: [
											...(env.points || []),
											...latestMove.automationData.points,
										].sort((a, b) => (a.time as number) - (b.time as number)),
										segments: [
											...(env.segments || []),
											...latestMove.automationData.segments,
										],
									};
								}
								return restored;
