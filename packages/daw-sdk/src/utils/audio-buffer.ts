 ...
 * Pure functions for audio format conversions
 */
...
export namespace audioBuffer {
	 ...
	 */
	export function toWav(buffer: AudioBuffer, opts: WavOptions): Uint8Array {
		...
		// Interleave
		const chans = Array.from({ length: numChannels }, (_, i) =>
			buffer.getChannelData(i),
		);
		let offset = headerSize;
		const ditherAmp = opts.dither
			? opts.bitDepth === 16
				? 1 / 65536
				: 1 / 16777216
			: 0;
